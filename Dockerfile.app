...
'' \
'[program:app]' \
'command=gunicorn src.main:app -c gunicorn.conf.py -w %(ENV_GUNICORN_WORKERS)s -k uvicorn.workers.UvicornWorker --bind 0.0.0.0:%(ENV_PORT)s --timeout 120 --access-logfile - --error-logfile - --log-level info --preload' \
'directory=/app' \
'autostart=true' \
'autorestart=true' \
