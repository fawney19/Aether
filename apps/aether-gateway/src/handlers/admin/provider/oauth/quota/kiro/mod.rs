...
pub(crate) async fn probe_kiro_provider_key_with_classification(
    ...
    let auth = match state.force_local_oauth_refresh_entry(&transport).await {
        ...
        Err(err) => {
            let message = kiro_quota_refresh_failure_message(&err);
            let status_code = kiro_quota_refresh_failure_status(&err);
    ...
    let result =
        match execute_kiro_quota_plan(state, &transport, &auth, proxy_override, probe_timeout)
            .await?
        {
    ...
    if result.status_code == 200 {
            ...
            .and_then(|body| body.json_body.as_ref())
        {
            ...
            if metadata_update.is_some() {
                ...
                let auth_config_json = serde_json::Value::Object(auth_config_object).to_string();
...
pub(crate) async fn refresh_kiro_provider_quota_locally(
    ...
    for key in keys {
        let outcome = probe_kiro_provider_key_with_classification(
            state,
            provider,
            endpoint,
            key,
