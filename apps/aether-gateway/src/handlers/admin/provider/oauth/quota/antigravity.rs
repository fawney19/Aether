...
async fn execute_antigravity_quota_plan(
    ...
    execute_provider_quota_plan(state, transport, plan, "antigravity", probe_timeout).await
}

pub(crate) async fn probe_antigravity_provider_key_with_classification(
    state: &AdminAppState<'_>,
...
pub(crate) async fn refresh_antigravity_provider_quota_locally(
    ...
    for key in keys {
        let outcome = probe_antigravity_provider_key_with_classification(
            state,
            provider,
            endpoint,
            key,
