...
#[test]
fn gateway_retries_next_local_openai_chat_stream_candidate_after_http_200_embedded_service_unavailable_error(
) {
    run_stream_decision_test(
        "gateway_retries_next_local_openai_chat_stream_candidate_after_http_200_embedded_service_unavailable_error",
        gateway_retries_next_local_openai_chat_stream_candidate_after_http_200_embedded_service_unavailable_error_impl,
    );
}

async fn gateway_retries_next_local_openai_chat_stream_candidate_after_http_200_embedded_service_unavailable_error_impl(
) {
    #[derive(Debug, Clone)]
    struct SeenExecutionRuntimeStreamRequest {
        trace_id: String,
    ...
    fn sample_candidate_row(
        ...
        StoredMinimalCandidateSelectionRow {
            ...
            model_id: format!("model-{provider_id}"),
            global_model_id: "global-model-openai-stream-embedded-failover".to_string(),
            global_model_name: "gpt-5".to_string(),
            global_model_mappings: None,
            global_model_supports_streaming: Some(true),
            model_provider_model_name: mapped_model.to_string(),
    ...
    fn sample_provider_catalog_key(
        ...
        .with_transport_fields(
            Some(serde_json::json!(["openai:chat"])),
            encrypt_python_fernet_plaintext(DEVELOPMENT_ENCRYPTION_KEY, key_value)
                .expect("api key should encrypt"),
            None,
            None,
            Some(serde_json::json!({ "openai:chat": priority })),
            None,
            None,
            None,
            None,
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "execution_runtime_candidate": true,
                    "auth_context": {
                        "user_id": "user-openai-local-stream-embedded-1",
                        "api_key_id": "api-key-openai-local-stream-embedded-1",
                        "access_allowed": true
                    },
                    "public_path": "/v1/chat/completions"
                }))
    ...
    let execution_runtime = Router::new().route(
        ...
        any(move |request: Request| {
            ...
            async move {
                ...
                let frames = if attempt == 1 {
                    // HTTP 200 with a nested provider error body: the prefetch
                    // inspection classifies this as EmbeddedError. Before the
                    // fix the gateway finalized this attempt as Responded and
                    // never retried the next candidate key.
                    concat!(
                        "{\"type\":\"headers\",\"payload\":{\"kind\":\"headers\",\"status_code\":200,\"headers\":{\"content-type\":\"application/json\"}}}\n",
                        "{\"type\":\"data\",\"payload\":{\"kind\":\"data\",\"text\":\"{\\\"error\\\":{\\\"code\\\":\\\"server_error\\\",\\\"type\\\":\\\"service_unavailable_error\\\",\\\"message\\\":\\\"Our servers are currently overloaded. Please try again later.\\\"}}\"}}\n",
                        "{\"type\":\"eof\",\"payload\":{\"kind\":\"eof\"}}\n"
                    )
                } else {
                    concat!(
                        "{\"type\":\"headers\",\"payload\":{\"kind\":\"headers\",\"status_code\":200,\"headers\":{\"content-type\":\"text/event-stream\"}}}\n",
                        "{\"type\":\"data\",\"payload\":{\"kind\":\"data\",\"text\":\"data: {\\\"id\\\":\\\"chatcmpl-local-stream-embedded-123\\\"}\\n\\n\"}}\n",
                        "{\"type\":\"data\",\"payload\":{\"kind\":\"data\",\"text\":\"data: [DONE]\\n\\n\"}}\n",
                        "{\"type\":\"telemetry\",\"payload\":{\"kind\":\"telemetry\",\"telemetry\":{\"elapsed_ms\":33,\"ttfb_ms\":12,\"upstream_bytes\":48}}}\n",
                        "{\"type\":\"eof\",\"payload\":{\"kind\":\"eof\"}}\n"
                    )
    ...
    let auth_repository = Arc::new(InMemoryAuthApiKeySnapshotRepository::seed(vec![(
        Some(hash_api_key("sk-client-openai-local-stream-embedded")),
        sample_auth_snapshot(
            "api-key-openai-local-stream-embedded-1",
            "user-openai-local-stream-embedded-1",
        ),
    )]));
    let candidate_selection_repository =
        Arc::new(InMemoryMinimalCandidateSelectionReadRepository::seed(vec![
            sample_candidate_row(
                "provider-openai-local-stream-embedded-primary",
                "endpoint-openai-local-stream-embedded-primary",
                "key-openai-local-stream-embedded-primary",
                10,
                1,
                "gpt-5-upstream-embedded-primary",
            ),
            sample_candidate_row(
                "provider-openai-local-stream-embedded-backup",
                "endpoint-openai-local-stream-embedded-backup",
                "key-openai-local-stream-embedded-backup",
                20,
                2,
                "gpt-5-upstream-embedded-backup",
            ),
        ]));
    let request_candidate_repository = Arc::new(InMemoryRequestCandidateRepository::default());
    let provider_catalog_repository = Arc::new(InMemoryProviderCatalogReadRepository::seed(
        ...
        vec![
            sample_provider_catalog_endpoint(
                "endpoint-openai-local-stream-embedded-primary",
                "provider-openai-local-stream-embedded-primary",
                "https://api.openai.embedded-primary.example",
            ),
            sample_provider_catalog_endpoint(
                "endpoint-openai-local-stream-embedded-backup",
                "provider-openai-local-stream-embedded-backup",
                "https://api.openai.embedded-backup.example",
            ),
        ],
        vec![
            sample_provider_catalog_key(
                "key-openai-local-stream-embedded-primary",
                "provider-openai-local-stream-embedded-primary",
                "sk-upstream-openai-embedded-primary",
                1,
            ),
            sample_provider_catalog_key(
                "key-openai-local-stream-embedded-backup",
                "provider-openai-local-stream-embedded-backup",
                "sk-upstream-openai-embedded-backup",
                2,
            ),
        ],
    ));
    ...
    let response = reqwest::Client::new()
        ...
        .header(
            http::header::AUTHORIZATION,
            "Bearer sk-client-openai-local-stream-embedded",
        )
        .header(
            TRACE_ID_HEADER,
            "trace-openai-chat-local-stream-embedded-123",
        )
        .body("{\"model\":\"gpt-5\",\"messages\":[],\"stream\":true}")
        .send()
        .await
    ...
    assert_eq!(
        strip_sse_keepalive_comments(&response.text().await.expect("body should read")),
        "data: {\"id\":\"chatcmpl-local-stream-embedded-123\"}\n\ndata: [DONE]\n\n"
    );
    ...
    assert_eq!(primary_request.model, "gpt-5-upstream-embedded-primary");
    assert!(primary_request.stream);
    assert_eq!(
        primary_request.authorization,
        "Bearer sk-upstream-openai-embedded-primary"
    );
    assert_eq!(
        backup_request.trace_id,
        "trace-openai-chat-local-stream-embedded-123"
    );
    assert_eq!(backup_request.model, "gpt-5-upstream-embedded-backup");
    assert_eq!(
        backup_request.authorization,
        "Bearer sk-upstream-openai-embedded-backup"
    );
    let stored_candidates = request_candidate_repository
        .list_by_request_id("trace-openai-chat-local-stream-embedded-123")
        .await
        .expect("request candidate trace should read");
    assert_eq!(stored_candidates.len(), 2);
    let failed_candidate = stored_candidates
        .iter()
        .find(|candidate| {
            candidate.status == RequestCandidateStatus::Failed && candidate.status_code == Some(503)
        })
        .expect("retryable embedded 503 failed candidate should be stored");
    let success_candidate = stored_candidates
        .iter()
        .find(|candidate| {
            candidate.status == RequestCandidateStatus::Success
    ...
    assert_eq!(failed_candidate.status, RequestCandidateStatus::Failed);
    assert_eq!(failed_candidate.status_code, Some(503));
    assert_eq!(
        failed_candidate.error_type.as_deref(),
...
fn gateway_retries_next_local_openai_chat_stream_candidate_after_http_200_embedded_error_split_across_frames(
    ...
    run_stream_decision_test(
        "gateway_retries_next_local_openai_chat_stream_candidate_after_http_200_embedded_error_split_across_frames",
        gateway_retries_next_local_openai_chat_stream_candidate_after_http_200_embedded_error_split_across_frames_impl,
    );
}

async fn gateway_retries_next_local_openai_chat_stream_candidate_after_http_200_embedded_error_split_across_frames_impl(
) {
    #[derive(Debug, Clone)]
    struct SeenExecutionRuntimeStreamRequest {
        trace_id: String,
        ...
        .route(
            ...
            any(|_request: Request| async move {
                Json(json!({
                    ...
                    "execution_runtime_candidate": true,
                    "auth_context": {
                        "user_id": "user-openai-local-stream-embedded-split-1",
                        "api_key_id": "api-key-openai-local-stream-embedded-split-1",
                        "access_allowed": true
                    },
                    "public_path": "/v1/chat/completions"
                }))
    ...
    let execution_runtime = Router::new().route(
        ...
        any(move |request: Request| {
            ...
            async move {
                ...
                let frames = if attempt == 1 {
                    // HTTP 200 with a nested provider error body split across
                    // many small data frames (8 bytes each, far more than
                    // MAX_STREAM_PREFETCH_FRAMES). Before the fix the gateway
                    // classified the first incomplete JSON frame as NonError
                    // and committed the candidate stream, losing failover.
                    let error_json = "{\"error\":{\"code\":\"server_error\",\"type\":\"service_unavailable_error\",\"message\":\"Our servers are currently overloaded. Please try again later.\"}}";
                    let mut frames = String::from(
                        "{\"type\":\"headers\",\"payload\":{\"kind\":\"headers\",\"status_code\":200,\"headers\":{\"content-type\":\"application/json\"}}}\n",
                    );
                    for chunk in error_json.as_bytes().chunks(8) {
                    ...
                    frames.push_str("{\"type\":\"eof\",\"payload\":{\"kind\":\"eof\"}}\n");
                    frames
                } else {
                    concat!(
                        "{\"type\":\"headers\",\"payload\":{\"kind\":\"headers\",\"status_code\":200,\"headers\":{\"content-type\":\"text/event-stream\"}}}\n",
                        "{\"type\":\"data\",\"payload\":{\"kind\":\"data\",\"text\":\"data: {\\\"id\\\":\\\"chatcmpl-local-stream-embedded-split-456\\\"}\\n\\n\"}}\n",
                        "{\"type\":\"data\",\"payload\":{\"kind\":\"data\",\"text\":\"data: [DONE]\\n\\n\"}}\n",
                        "{\"type\":\"telemetry\",\"payload\":{\"kind\":\"telemetry\",\"telemetry\":{\"elapsed_ms\":33,\"ttfb_ms\":12,\"upstream_bytes\":48}}}\n",
                        "{\"type\":\"eof\",\"payload\":{\"kind\":\"eof\"}}\n"
                    )
    ...
    let auth_repository = Arc::new(InMemoryAuthApiKeySnapshotRepository::seed(vec![(
        Some(hash_api_key("sk-client-openai-local-stream-embedded-split")),
        sample_auth_snapshot(
            "api-key-openai-local-stream-embedded-split-1",
            "user-openai-local-stream-embedded-split-1",
        ),
    )]));
    let candidate_selection_repository =
        Arc::new(InMemoryMinimalCandidateSelectionReadRepository::seed(vec![
            sample_candidate_row(
                "provider-openai-local-stream-embedded-split-primary",
                "endpoint-openai-local-stream-embedded-split-primary",
                "key-openai-local-stream-embedded-split-primary",
                10,
                1,
                "gpt-5-upstream-embedded-split-primary",
            ),
            sample_candidate_row(
                "provider-openai-local-stream-embedded-split-backup",
                "endpoint-openai-local-stream-embedded-split-backup",
                "key-openai-local-stream-embedded-split-backup",
                20,
                2,
                "gpt-5-upstream-embedded-split-backup",
            ),
        ]));
    let request_candidate_repository = Arc::new(InMemoryRequestCandidateRepository::default());
    let provider_catalog_repository = Arc::new(InMemoryProviderCatalogReadRepository::seed(
        ...
        vec![
            sample_provider_catalog_endpoint(
                "endpoint-openai-local-stream-embedded-split-primary",
                "provider-openai-local-stream-embedded-split-primary",
                "https://api.openai.embedded-split-primary.example",
            ),
            sample_provider_catalog_endpoint(
                "endpoint-openai-local-stream-embedded-split-backup",
                "provider-openai-local-stream-embedded-split-backup",
                "https://api.openai.embedded-split-backup.example",
            ),
        ],
        vec![
            sample_provider_catalog_key(
                "key-openai-local-stream-embedded-split-primary",
                "provider-openai-local-stream-embedded-split-primary",
                "sk-upstream-openai-embedded-split-primary",
                1,
            ),
            sample_provider_catalog_key(
                "key-openai-local-stream-embedded-split-backup",
                "provider-openai-local-stream-embedded-split-backup",
                "sk-upstream-openai-embedded-split-backup",
                2,
            ),
        ],
    ));
    ...
    let response = reqwest::Client::new()
        ...
        .header(
            http::header::AUTHORIZATION,
            "Bearer sk-client-openai-local-stream-embedded-split",
        )
        .header(
            TRACE_ID_HEADER,
    ...
    assert_eq!(
        strip_sse_keepalive_comments(&response.text().await.expect("body should read")),
        "data: {\"id\":\"chatcmpl-local-stream-embedded-split-456\"}\n\ndata: [DONE]\n\n"
    );
    ...
    let primary_request = seen_execution_runtime_requests
        .iter()
        .find(|request| {
            request.url == "https://api.openai.embedded-split-primary.example/chat/completions"
        })
        .expect("primary execution runtime request should be captured");
    let backup_request = seen_execution_runtime_requests
        .iter()
        .find(|request| {
            request.url == "https://api.openai.embedded-split-backup.example/chat/completions"
        })
        .expect("backup execution runtime request should be captured");
    assert_eq!(
        primary_request.trace_id,
    ...
    assert_eq!(
        primary_request.authorization,
        "Bearer sk-upstream-openai-embedded-split-primary"
    );
    assert_eq!(
        backup_request.trace_id,
        "trace-openai-chat-local-stream-embedded-split-split-456"
    );
    assert_eq!(backup_request.model, "gpt-5-upstream-embedded-split-backup");
    assert_eq!(
        backup_request.authorization,
        "Bearer sk-upstream-openai-embedded-split-backup"
    );
    let stored_candidates = request_candidate_repository
        .list_by_request_id("trace-openai-chat-local-stream-embedded-split-split-456")
        .await
        .expect("request candidate trace should read");
    assert_eq!(stored_candidates.len(), 2);
    let failed_candidate = stored_candidates
