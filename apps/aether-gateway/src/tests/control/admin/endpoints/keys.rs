...
async fn gateway_handles_admin_keys_grouped_by_format_locally_with_trusted_admin_principal() {
    ...
    // Regression guard for issue #505: priority edits persisted in the DB but
    // the dialog reopened with placeholder display order because the grouped
    // endpoint returned summary rows with priority fields nulled out.
