...
pub(crate) struct RedisRedactionMappingCache<'a> {
    runner: &'a RedisKvRunner,
    key_prefix: String,
}

impl<'a> RedisRedactionMappingCache<'a> {
    pub(crate) fn new(runner: &'a RedisKvRunner) -> Self {
        Self {
            runner,
    ...
    pub(crate) async fn lookup_sentinel(
        ...
        let sentinel = self
            ...
            .await?;
        Ok(sentinel.filter(|value| value.len() <= MAX_CACHE_SENTINEL_BYTES))
    }
    ...
    async fn get_string(&self, key: &str) -> Result<Option<String>, DataLayerError> {
        let mut connection = self
            ...
            .await
            .map_err(DataLayerError::redis)?;
        let namespaced_key = self.runner.keyspace().key(key);
        redis::cmd("GET")
            .arg(&namespaced_key)
...
fn push_regex_candidates(
    ...
    for matched in regex.find_iter(input) {
        ...
        if has_token_boundary(input, matched.start(), matched.end()) && validator(value) {
...
fn parse_enabled_redaction_kinds(value: Option<&Value>) -> HashSet<RedactionKind> {
    ...
    let kinds = values
        ...
        .collect::<HashSet<_>>();
    if kinds.is_empty() {
        default_enabled_redaction_kinds()
    } else {
        kinds
    }
}
...
mod tests {
    use super::{
        build_redaction_session_config, mask_chat_request_json,
        mask_chat_request_json_with_options, provider_chat_pii_redaction_enabled,
        restore_sync_response_body, try_mask_chat_request_json_with_cache_options,
        try_mask_chat_request_json_with_options, ChatPiiRedactionProviderScope,
        ChatPiiRedactionRuntimeConfig, MappingKey, MaskChatRequestOptions, RedactionKind,
        RedactionLimitError, RedactionMapping, RedactionScanLimits, RedactionSession,
        RedactionSessionConfig, RedisRedactionMappingCache, StreamingResponseRestorer,
    };
    use std::collections::BTreeMap;

    use aether_data::redis::{
        RedisClientConfig, RedisClientFactory, RedisKvRunner, RedisKvRunnerConfig,
    };
    ...
    async fn start_managed_redis_or_skip() -> Option<ManagedRedisServer> {
        match ManagedRedisServer::start().await {
            ...
            Err(err) if err.to_string().contains("No such file or directory") => {
                eprintln!("skipping redis-backed pii redaction test: {err}");
                None
            }
    ...
    fn redis_runner(redis_url: &str, key_prefix: &str) -> RedisKvRunner {
        let factory = RedisClientFactory::new(RedisClientConfig {
            url: redis_url.to_string(),
            key_prefix: Some(key_prefix.to_string()),
        })
        .expect("redis factory should build");
        RedisKvRunner::new(
            factory.connect_lazy().expect("redis client should build"),
            factory.config().keyspace(),
            RedisKvRunnerConfig::default(),
        )
        .expect("redis runner should build")
    }
    ...
    fn proxy_pii_redaction_provider_bound_request_uses_sentinels_and_inserts_safe_notice() {
        ...
        let masked_json: serde_json::Value =
            serde_json::from_slice(&masked.body).expect("masked request should parse");
        assert_eq!(masked_json["messages"][0]["role"], "system");
        assert_eq!(masked_json["messages"][1]["role"], "assistant");
        let notice = masked_json["messages"][1]["content"]
            .as_str()
        ...
        assert_eq!(masked_json["messages"][2]["role"], "user");
        let content = masked_json["messages"][2]["content"]
            .as_str()
            .expect("user content should be text");
        assert!(!content.contains("alice@example.com"));
        assert!(content.contains("<AETHER:EMAIL:"));
        assert_eq!(masked_json["messages"][3]["role"], "assistant");
    }
    ...
    async fn pii_redaction_performance_multiturn_cache_reuses_and_rolls_over_sentinels() {
        ...
        let runner = redis_runner(redis.redis_url(), "pii_redaction_performance_cache");
        let cache = RedisRedactionMappingCache::new(&runner);
        let config = ChatPiiRedactionRuntimeConfig {
            enabled_kinds: [RedactionKind::Email].into_iter().collect(),
            ..ChatPiiRedactionRuntimeConfig::default()
        };
        ...
        let keys = redis::cmd("KEYS")
            .arg(runner.keyspace().key("privacy:redaction:mapping:*"))
        ...
        for key in keys {
            ...
            assert!(ttl > 0, "redaction cache key {key} should have ttl");
        }
    }
