...
from src.models.database import PaymentCallback, PaymentOrder, RefundRequest, Wallet, WalletTransaction
...
def serialize_payment_order(
    ...
    *,
    ...
    return {
        ...
        "exchange_rate": float(order.exchange_rate or 0) if order.exchange_rate is not None else None,
...
def serialize_wallet_transaction(tx: WalletTransaction) -> dict[str, Any]:
    return {
        ...
        "recharge_balance_before": (
            float(tx.recharge_balance_before) if tx.recharge_balance_before is not None else None
        ),
        "recharge_balance_after": (
            float(tx.recharge_balance_after) if tx.recharge_balance_after is not None else None
        ),
        "gift_balance_before": float(tx.gift_balance_before) if tx.gift_balance_before is not None else None,
        "gift_balance_after": float(tx.gift_balance_after) if tx.gift_balance_after is not None else None,
...
def _wallet_owner(wallet: Wallet | None) -> tuple[str, str | None]:
    owner_type = "user" if wallet and wallet.user_id else "api_key"
    ...
    if wallet is not None:
        ...
        elif wallet.api_key_id:
            owner_name = f"Key-{wallet.api_key_id[:8]}"
    return owner_type, owner_name
