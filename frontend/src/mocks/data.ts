...
export const MOCK_SYSTEM_CONFIGS: Array<{ key: string; value: unknown; description?: string }> = [
  ...
  { key: 'module.external_integrations.enabled', value: false, description: '外部系统集成开关' },
  {
    key: 'module.external_integrations.items',
    value: [],
    description: '外部系统入口配置',
  },
...
const MOCK_MODULE_DEFINITIONS: Array<Omit<ModuleStatus, 'active' | 'health'> & { health?: ModuleStatus['health'] }> = [
  ...
  },
  {
    ...
    enabled: false,
    config_validated: false,
    config_error: '请先配置至少一个已启用的外部系统入口',
    admin_route: '/admin/modules/external-integrations',
    admin_menu_icon: 'ExternalLink',
    admin_menu_group: 'system',
    admin_menu_order: 61,
  },
  {
    ...
    description: '将用户、计费、风控和上游异常事件可靠投递到外部系统',
    category: 'integration',
    available: true,
    enabled: true,
    config_validated: true,
    config_error: null,
    admin_route: '/admin/modules/webhook-outbound',
    admin_menu_icon: 'Webhook',
    admin_menu_group: 'system',
    admin_menu_order: 62,
  },
  {
