...
#[derive(Debug, Clone)]
pub struct MysqlWebhookNotificationRepository {
    pool: MysqlPool,
}

impl MysqlWebhookNotificationRepository {
...
#[async_trait]
impl WebhookNotificationReadRepository for MysqlWebhookNotificationRepository {
    async fn list_webhook_endpoints(
        &self,
        query: &WebhookEndpointListQuery,
    ) -> Result<Vec<StoredWebhookEndpoint>, DataLayerError> {
        let mut builder = QueryBuilder::<MySql>::new(ENDPOINT_COLUMNS);
    ...
    async fn list_webhook_deliveries(
        ...
        let mut count_builder =
            QueryBuilder::<MySql>::new("SELECT COUNT(id) AS total FROM webhook_deliveries");
        ...
        let mut builder = QueryBuilder::<MySql>::new(DELIVERY_COLUMNS);
...
#[async_trait]
impl WebhookNotificationWriteRepository for MysqlWebhookNotificationRepository {
...
ON DUPLICATE KEY UPDATE
  ...
  event_filter_json = VALUES(event_filter_json),
  timeout_ms = VALUES(timeout_ms),
  max_attempts = VALUES(max_attempts),
  initial_retry_delay_seconds = VALUES(initial_retry_delay_seconds),
  max_retry_delay_seconds = VALUES(max_retry_delay_seconds),
...
ON DUPLICATE KEY UPDATE updated_at_unix_secs = updated_at_unix_secs
"#,
        )
        .bind(&id)
...
ON DUPLICATE KEY UPDATE
  ...
  completed_at_unix_secs = VALUES(completed_at_unix_secs),
...
fn apply_delivery_filter(builder: &mut QueryBuilder<'_, MySql>, query: &WebhookDeliveryListQuery) {
    let mut has_where = false;
...
fn map_endpoint_row(row: &MySqlRow) -> Result<StoredWebhookEndpoint, DataLayerError> {
    Ok(StoredWebhookEndpoint {
        ...
        event_filter_json: parse_optional_json(row.try_get("event_filter_json").ok().flatten())?,
...
fn map_delivery_row(row: &MySqlRow) -> Result<StoredWebhookDelivery, DataLayerError> {
...
fn map_attempt_row(row: &MySqlRow) -> Result<StoredWebhookDeliveryAttempt, DataLayerError> {
...
fn parse_required_json(
    ...
    let raw = value.ok_or_else(|| {
        DataLayerError::UnexpectedValue(format!("webhook delivery {field} is missing"))
    })?;
    serde_json::from_str::<serde_json::Value>(&raw).map_err(DataLayerError::sql)
}
