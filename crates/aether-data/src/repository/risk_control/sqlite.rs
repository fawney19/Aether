...
#[derive(Debug, Clone)]
pub struct SqliteRiskControlRepository {
    pool: SqlitePool,
}

impl SqliteRiskControlRepository {
    ...
    async fn list_notification_outbox_by_ids(
        ...
        let mut builder = QueryBuilder::<Sqlite>::new(OUTBOX_SELECT_BASE);
...
#[async_trait]
impl RiskControlReadRepository for SqliteRiskControlRepository {
    async fn list_logs(
        &self,
        query: &RiskControlLogListQuery,
    ) -> Result<StoredRiskControlLogPage, DataLayerError> {
        let mut count_builder =
            QueryBuilder::<Sqlite>::new("SELECT COUNT(id) AS total FROM risk_control_logs");
        ...
        let mut list_builder = QueryBuilder::<Sqlite>::new(LOG_SELECT_BASE);
...
"#,
    ...
    async fn list_flagged_hashes(
        ...
        let mut builder = QueryBuilder::<Sqlite>::new(HASH_SELECT_BASE);
...
#[async_trait]
impl RiskControlWriteRepository for SqliteRiskControlRepository {
    async fn insert_log(
        &self,
...
ON CONFLICT(input_hash) DO UPDATE SET
  ...
  highest_score = MAX(excluded.highest_score, risk_control_flagged_hashes.highest_score),
...
fn apply_log_filters(builder: &mut QueryBuilder<'_, Sqlite>, query: &RiskControlLogListQuery) {
...
fn map_log_row(row: &SqliteRow) -> Result<StoredRiskControlLog, DataLayerError> {
...
fn map_hash_row(row: &SqliteRow) -> Result<StoredRiskControlFlaggedHash, DataLayerError> {
...
fn map_outbox_row(row: &SqliteRow) -> Result<StoredRiskControlNotificationOutbox, DataLayerError> {
