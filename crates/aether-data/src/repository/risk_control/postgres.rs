...
#[derive(Debug, Clone)]
pub struct SqlxRiskControlRepository {
    pool: PostgresPool,
}

impl SqlxRiskControlRepository {
...
FROM public.risk_control_flagged_hashes
"#;
...
FROM public.risk_control_notification_outbox
"#;
...
#[async_trait]
impl RiskControlReadRepository for SqlxRiskControlRepository {
    async fn list_logs(
        &self,
        query: &RiskControlLogListQuery,
    ) -> Result<StoredRiskControlLogPage, DataLayerError> {
        let mut count_builder = QueryBuilder::<Postgres>::new(
            "SELECT COUNT(id) AS total FROM public.risk_control_logs",
        );
        ...
        let mut list_builder = QueryBuilder::<Postgres>::new(LOG_SELECT_BASE);
...
SELECT COUNT(id)
FROM public.risk_control_logs
WHERE flagged IS TRUE AND user_id = $1 AND created_at >= $2
"#,
        )
        .bind(user_id)
    ...
    async fn list_flagged_hashes(
        ...
        let mut builder = QueryBuilder::<Postgres>::new(HASH_SELECT_BASE);
    ...
    async fn count_flagged_hashes(&self) -> Result<u64, DataLayerError> {
        let total = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(input_hash) FROM public.risk_control_flagged_hashes",
        )
        .fetch_one(&self.pool)
        .await
        .map_postgres_err()?;
    ...
    async fn list_notification_outbox_by_log_ids(
        ...
        let mut builder = QueryBuilder::<Postgres>::new(OUTBOX_SELECT_BASE);
...
SELECT
  ...
  COUNT(CASE WHEN status = 'sent' THEN 1 END)::BIGINT AS sent,
  COUNT(CASE WHEN status = 'dead' THEN 1 END)::BIGINT AS dead,
...
"#,
        ...
        Ok(RiskControlNotificationOutboxSummary {
            pending: u64_from_i64(row.try_get("pending").map_postgres_err()?),
            processing: u64_from_i64(row.try_get("processing").map_postgres_err()?),
            sent: u64_from_i64(row.try_get("sent").map_postgres_err()?),
            dead: u64_from_i64(row.try_get("dead").map_postgres_err()?),
            oldest_pending_at_unix_secs: optional_u64_from_i64(
                row.try_get("oldest_pending_at").map_postgres_err()?,
            ),
            next_attempt_at_unix_secs: optional_u64_from_i64(
                row.try_get("next_attempt_at").map_postgres_err()?,
            ),
            last_error,
        })
    }
...
#[async_trait]
impl RiskControlWriteRepository for SqlxRiskControlRepository {
    async fn insert_log(
        &self,
        ...
        sqlx::query(
            r#"
INSERT INTO public.risk_control_logs (
  id, trace_id, request_id, user_id, username, user_email, api_key_id, api_key_name,
  route_family, route_kind, api_format, endpoint, model, mode, action, decision_source,
  flagged, highest_category, highest_score, category_scores, thresholds, matched_keywords,
  input_hash, excerpt, latency_ms, queue_delay_ms, violation_count, auto_action,
...
"#,
        ...
        .bind(record.category_scores)
        .bind(record.thresholds)
        .bind(record.matched_keywords)
        ...
        .bind(
            record
                .notification_last_attempt_at_unix_secs
    ...
    async fn mark_log_notification_sent(&self, id: &str) -> Result<bool, DataLayerError> {
        let result = sqlx::query(
            "UPDATE public.risk_control_logs SET notification_sent = TRUE, notification_last_error = NULL WHERE id = $1",
        )
        .bind(id)
        .execute(&self.pool)
        .await
    ...
    async fn record_log_notification_result(
        ...
        let result = sqlx::query(
            r#"
UPDATE public.risk_control_logs
SET
  notification_sent = CASE WHEN $2 THEN TRUE ELSE notification_sent END,
  notification_attempts = notification_attempts + 1,
  notification_last_error = $3,
  notification_last_attempt_at = $4
WHERE id = $1
"#,
        )
        .bind(id)
    ...
    async fn upsert_flagged_hash(
        ...
        sqlx::query(
            r#"
INSERT INTO public.risk_control_flagged_hashes (
  input_hash, source_log_id, reason, highest_category, highest_score, excerpt,
  first_seen_at, last_seen_at, hit_count
)
VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 1)
ON CONFLICT(input_hash) DO UPDATE SET
  source_log_id = COALESCE(EXCLUDED.source_log_id, risk_control_flagged_hashes.source_log_id),
  reason = COALESCE(EXCLUDED.reason, risk_control_flagged_hashes.reason),
  highest_category = COALESCE(EXCLUDED.highest_category, risk_control_flagged_hashes.highest_category),
  highest_score = GREATEST(EXCLUDED.highest_score, risk_control_flagged_hashes.highest_score),
  excerpt = COALESCE(EXCLUDED.excerpt, risk_control_flagged_hashes.excerpt),
  last_seen_at = EXCLUDED.last_seen_at,
  hit_count = risk_control_flagged_hashes.hit_count + 1
"#,
        )
        .bind(&record.input_hash)
    ...
    async fn delete_flagged_hash(&self, input_hash: &str) -> Result<bool, DataLayerError> {
        let result =
            sqlx::query("DELETE FROM public.risk_control_flagged_hashes WHERE input_hash = $1")
                .bind(input_hash)
                .execute(&self.pool)
                .await
                .map_postgres_err()?;
    ...
    async fn clear_flagged_hashes(&self) -> Result<u64, DataLayerError> {
        let result = sqlx::query("DELETE FROM public.risk_control_flagged_hashes")
            .execute(&self.pool)
            .await
            .map_postgres_err()?;
    ...
    async fn delete_expired_logs(
        ...
        let hit_deleted = match hit_before_unix_secs {
            Some(cutoff) => sqlx::query(
                "DELETE FROM public.risk_control_logs WHERE flagged IS TRUE AND created_at < $1",
            )
            .bind(cutoff as i64)
            .execute(&self.pool)
            .await
        ...
        let non_hit_deleted = match non_hit_before_unix_secs {
            Some(cutoff) => sqlx::query(
                "DELETE FROM public.risk_control_logs WHERE flagged IS FALSE AND created_at < $1",
            )
            .bind(cutoff as i64)
            .execute(&self.pool)
            .await
    ...
    async fn insert_notification_outbox(
        ...
        sqlx::query(
            r#"
INSERT INTO public.risk_control_notification_outbox (
  id, log_id, item_key, title, markdown_body, text_body, variables_json, status,
  attempt_count, max_attempts, next_attempt_at, lease_until, last_error, created_at, updated_at, sent_at
)
VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending', 0, $8, $9, NULL, NULL, $10, $11, NULL)
ON CONFLICT(log_id, item_key) DO UPDATE SET
  id = EXCLUDED.id,
...
UPDATE public.risk_control_notification_outbox AS outbox
...
RETURNING
  outbox.id, outbox.log_id, outbox.item_key, outbox.title, outbox.markdown_body,
  outbox.text_body, outbox.variables_json, outbox.status, outbox.attempt_count,
  outbox.max_attempts, outbox.next_attempt_at, outbox.lease_until, outbox.last_error,
  outbox.created_at, outbox.updated_at, outbox.sent_at
"#,
        )
    ...
    async fn record_notification_outbox_attempt(
        ...
        let result = sqlx::query(
            r#"
UPDATE public.risk_control_notification_outbox
SET
  status = $2,
...
SET
  ...
  updated_at = $2,
  sent_at = NULL
WHERE log_id = $1 AND status <> 'sent'
"#,
        )
        .bind(log_id)
        .bind(next_attempt_at_unix_secs as i64)
...
fn apply_log_filters(builder: &mut QueryBuilder<'_, Postgres>, query: &RiskControlLogListQuery) {
...
fn map_log_row(row: &PgRow) -> Result<StoredRiskControlLog, DataLayerError> {
    Ok(StoredRiskControlLog {
        id: row.try_get("id").map_postgres_err()?,
        trace_id: row.try_get("trace_id").map_postgres_err()?,
        ...
        route_family: row.try_get("route_family").map_postgres_err()?,
        route_kind: row.try_get("route_kind").map_postgres_err()?,
        api_format: row.try_get("api_format").map_postgres_err()?,
        endpoint: row.try_get("endpoint").map_postgres_err()?,
        model: row.try_get("model").map_postgres_err()?,
        mode: row.try_get("mode").map_postgres_err()?,
        action: row.try_get("action").map_postgres_err()?,
        decision_source: row.try_get("decision_source").map_postgres_err()?,
        flagged: row.try_get("flagged").map_postgres_err()?,
        highest_category: row.try_get("highest_category").map_postgres_err()?,
        highest_score: row.try_get("highest_score").map_postgres_err()?,
        category_scores: row.try_get("category_scores").map_postgres_err()?,
        thresholds: row.try_get("thresholds").map_postgres_err()?,
        matched_keywords: row.try_get("matched_keywords").map_postgres_err()?,
        input_hash: row.try_get("input_hash").map_postgres_err()?,
        excerpt: row.try_get("excerpt").map_postgres_err()?,
        latency_ms: optional_u64_from_i64(row.try_get("latency_ms").map_postgres_err()?),
        queue_delay_ms: optional_u64_from_i64(row.try_get("queue_delay_ms").map_postgres_err()?),
        violation_count: u64_from_i64(row.try_get("violation_count").map_postgres_err()?),
        auto_action: row.try_get("auto_action").map_postgres_err()?,
        notification_sent: row.try_get("notification_sent").map_postgres_err()?,
        notification_attempts: u64_from_i64(
            row.try_get("notification_attempts").map_postgres_err()?,
        ),
        notification_last_error: row.try_get("notification_last_error").map_postgres_err()?,
        notification_last_attempt_at_unix_secs: optional_u64_from_i64(
            row.try_get("notification_last_attempt_at")
                .map_postgres_err()?,
        ),
        error_message: row.try_get("error_message").map_postgres_err()?,
        created_at_unix_secs: u64_from_i64(row.try_get("created_at").map_postgres_err()?),
    })
}

fn map_hash_row(row: &PgRow) -> Result<StoredRiskControlFlaggedHash, DataLayerError> {
    Ok(StoredRiskControlFlaggedHash {
        input_hash: row.try_get("input_hash").map_postgres_err()?,
        source_log_id: row.try_get("source_log_id").map_postgres_err()?,
        reason: row.try_get("reason").map_postgres_err()?,
        highest_category: row.try_get("highest_category").map_postgres_err()?,
        highest_score: row.try_get("highest_score").map_postgres_err()?,
        excerpt: row.try_get("excerpt").map_postgres_err()?,
        first_seen_at_unix_secs: u64_from_i64(row.try_get("first_seen_at").map_postgres_err()?),
        last_seen_at_unix_secs: u64_from_i64(row.try_get("last_seen_at").map_postgres_err()?),
        hit_count: u64_from_i64(row.try_get("hit_count").map_postgres_err()?),
    })
}

fn map_outbox_row(row: &PgRow) -> Result<StoredRiskControlNotificationOutbox, DataLayerError> {
    Ok(StoredRiskControlNotificationOutbox {
        id: row.try_get("id").map_postgres_err()?,
        log_id: row.try_get("log_id").map_postgres_err()?,
        item_key: row.try_get("item_key").map_postgres_err()?,
        title: row.try_get("title").map_postgres_err()?,
        markdown_body: row.try_get("markdown_body").map_postgres_err()?,
        text_body: row.try_get("text_body").map_postgres_err()?,
        variables_json: row.try_get("variables_json").map_postgres_err()?,
        status: row.try_get("status").map_postgres_err()?,
        attempt_count: u64_from_i64(row.try_get("attempt_count").map_postgres_err()?),
        max_attempts: u64_from_i64(row.try_get("max_attempts").map_postgres_err()?),
        next_attempt_at_unix_secs: optional_u64_from_i64(
            row.try_get("next_attempt_at").map_postgres_err()?,
        ),
        lease_until_unix_secs: optional_u64_from_i64(
            row.try_get("lease_until").map_postgres_err()?,
        ),
        last_error: row.try_get("last_error").map_postgres_err()?,
        created_at_unix_secs: u64_from_i64(row.try_get("created_at").map_postgres_err()?),
        updated_at_unix_secs: u64_from_i64(row.try_get("updated_at").map_postgres_err()?),
        sent_at_unix_secs: optional_u64_from_i64(row.try_get("sent_at").map_postgres_err()?),
    })
}
