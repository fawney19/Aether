...
static PROCESS_SESSION_ID: LazyLock<String> = LazyLock::new(|| generate_prefixed_id("ses_"));
...
pub fn build_opencode_free_headers() -> BTreeMap<String, String> {
    ...
    BTreeMap::from([
        (String::from("x-opencode-session"), session_id.to_string()),
        (String::from("x-opencode-request"), request_id),
        (
...
mod tests {
    ...
    fn test_session_id_format() {
        ...
        assert!(
            id.starts_with("ses_"),
            "session ID should start with ses_, got: {id}"
        );
        let body = &id[4..];
        assert_eq!(
            body.len(),
            26,
            "session ID body should be 26 chars, got {}: len={}",
            body,
            body.len()
        );
    ...
    fn test_request_id_format() {
        ...
        assert!(
            id.starts_with("msg_"),
            "request ID should start with msg_, got: {id}"
        );
        let body = &id[4..];
        assert_eq!(
            body.len(),
            26,
            "request ID body should be 26 chars, got {}: len={}",
            body,
            body.len()
        );
        let (hex_part, base62_part) = body.split_at(12);
